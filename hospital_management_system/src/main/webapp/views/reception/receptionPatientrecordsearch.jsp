    ...
    <script>
        ...
        function loadAllPatients() {
            fetch('ReceptionPatientServlet?query=')
                ...
                .catch(error => {
                    ...
                    const tbody = document.getElementById("patientResultsTbody");
                    tbody.innerHTML = "<tr><td colspan='9' class='no-results'>Error loading patients</td></tr>";
                });
        }
        ...
        function livePatientSearch() {
            ...
            fetch('ReceptionPatientServlet?query=' + encodeURIComponent(input))
                ...
                .catch(error => {
                    console.error('Error:', error);
                    tbody.innerHTML = "<tr><td colspan='9' class='no-results'>Error fetching data</td></tr>";
                });
        }
        ...
        function renderTableRows(data, tbody) {
            ...
            if (!data || data.length === 0) {
                tbody.innerHTML = "<tr><td colspan='9' class='no-results'>No patients found</td></tr>";
                return;
            }
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                tableContent += '<tr>' +
                    ...
                    '<td data-label="DOB">' + (patient.patientDob || '') + '</td>' +
                    '<td data-label="Clinic ID">' + (patient.clinicId || '') + '</td>' +
                    '<td data-label="NIC">' + (patient.patientNic || '') + '</td>' +
                    '</tr>';
            }
